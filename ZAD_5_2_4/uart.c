...
void Uart_GetString(char *acBuffer){
	ReceiverBuffer asReceiverBuffer;
	char cCharBuffer;
	asReceiverBuffer.ucCharCtr = 0;
	
	xQueueReceive(xQueue,&cCharBuffer,portMAX_DELAY);
	while(cCharBuffer != CR){
		if(asReceiverBuffer.ucCharCtr < RECIEVER_SIZE){
			asReceiverBuffer.cData[asReceiverBuffer.ucCharCtr] = cCharBuffer;
			asReceiverBuffer.ucCharCtr++;
		}
		xQueueReceive(xQueue,&cCharBuffer,portMAX_DELAY);
	}
	asReceiverBuffer.cData[asReceiverBuffer.ucCharCtr] = NULL;
	CopyString(asReceiverBuffer.cData,acBuffer);
}
